...
function processUserInput(promptText, presetName) {
  try {
    ...
    // フォルダ移動処理（エラー時はマイドライブに維持）
    if (settings.folderUrl && String(settings.folderUrl).trim() !== "") {
      try {
        ...
        if (folderId) {
          ...
          Logger.log('URLから有効なフォルダIDを取得できませんでした。マイドライブに保存します。');
        }
      } catch (folderError) {
        // 権限エラーやID間違いなどの場合はログに残し、処理を続行（＝マイドライブに残す）
        Logger.log('フォルダ移動に失敗しました（マイドライブに保存されます）: ' + folderError.message);
      }
    } else {
       Logger.log('保存先URLが空欄のため、マイドライブに保存します。');
    }
...
function getFolderIdFromUrl(url) {
  ...
  const match = urlStr.match(/\/folders\/([a-zA-Z0_9-_]+)/);
  if (match) return match[1];
  
  // ID直打ちのパターンにも対応
  const idOnlyMatch = urlStr.match(/^[a-zA-Z0_9-_]{20,}$/);
  if (idOnlyMatch) return urlStr;
